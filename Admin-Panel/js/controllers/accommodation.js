myApp.controller('AccommodationController', ['$scope', '$firebaseArray', function($scope, $firebaseArray) {
  var myAccommodation = new Firebase('https://discovermalawi.firebaseio.com/accommodation');
    $scope.accommodation = $firebaseArray(myAccommodation);
    ...
    $scope.editFormSubmit = function(){
        ...
        var record = $scope.accommodation.$getRecord(id); //to store the whole park we're editing
    ...
    $scope.deletePark = function(item){
        $scope.accommodation.$remove(item);
